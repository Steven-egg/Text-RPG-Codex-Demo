...
function loadFixture(scenarioKey) {
  ...
  try {
    let model = null;
    const key = scenarioKey.replace('synthesis-', '').replace('.json', '');
    if (key === "default") {
      model = JSON.parse(JSON.stringify(window.SYNTHESIS_DEFAULT_FIXTURE));
    } else if (key === "constrained") {
      model = JSON.parse(JSON.stringify(window.SYNTHESIS_CONSTRAINED_FIXTURE));
    } else {
