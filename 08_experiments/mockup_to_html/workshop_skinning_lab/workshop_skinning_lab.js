(function () {
  /**
   * 邊境工坊 - Workshop Screen Prototype Logic
  ...
  function loadFixture(scenarioKey) {
    try {
      let model = null;
      const key = scenarioKey.replace('workshop-', '').replace('.json', '');
      if (key === "default") {
        model = JSON.parse(JSON.stringify(window.WORKSHOP_DEFAULT_FIXTURE));
      } else if (key === "constrained") {
        model = JSON.parse(JSON.stringify(window.WORKSHOP_CONSTRAINED_FIXTURE));
      } else {
        throw new Error("Unknown scenario key: " + scenarioKey);
      }
      currentFixtureData = model;
      logUIAction('load_fixture', { file: scenarioKey, player: model.player.name, job: model.player.job });
      ...
      console.error(error);
      feedbackBar.textContent = `讀取 Fixture 失敗: ${error.message}`;
