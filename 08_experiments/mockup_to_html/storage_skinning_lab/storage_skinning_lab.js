...
function loadFixture(scenarioKey) {
  ...
  try {
    let model = null;
    const key = scenarioKey.replace('storage-', '').replace('.json', '');
    if (key === "locked") {
      model = JSON.parse(JSON.stringify(window.STORAGE_LOCKED_FIXTURE));
    } else if (key === "empty") {
      model = JSON.parse(JSON.stringify(window.STORAGE_EMPTY_FIXTURE));
    } else if (key === "filled") {
      model = JSON.parse(JSON.stringify(window.STORAGE_FILLED_FIXTURE));
    } else if (key === "blocked") {
      model = JSON.parse(JSON.stringify(window.STORAGE_BLOCKED_FIXTURE));
    } else {
