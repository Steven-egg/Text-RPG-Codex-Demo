/**
 * Shop Skinning Lab v0.1.1 — JavaScript Behavior Controller
 * Replicates the rendering, interactions, keyboard navigation, and action logging
 * of the official Shop Screen prototype, powered completely by local in-memory fixtures.
 * 
 * Contains NO external ES module imports, runtimeClient references, or fetch calls.
 */
...
(function () {
  ...
  backActionEl.addEventListener("click", () => {
    ...
    renderFeedback("系統", "「返回城鎮已記錄（Lab 模式不執行真實導航與頁面跳轉）。」");
  });
  ...
  function loadFixture(scenarioKey) {
    ...
    try {
      ...
      if (scenarioKey === "default") {
        model = JSON.parse(JSON.stringify(window.SHOP_DEFAULT_FIXTURE));
      } else if (scenarioKey === "constrained") {
        model = JSON.parse(JSON.stringify(window.SHOP_CONSTRAINED_FIXTURE));
      } else {
  ...
  function render() {
    ...
    // Replicate resource strip behavior (defaults to hidden in official Shop screen)
  ...
  function renderPrimaryAction(action) {
    ...
    primaryActionEl.textContent = normalized.label ?? "購買商品";
  ...
  function renderLoadError(error) {
    ...
    renderFeedback("系統", "請確認 fixtures.js 載入狀態。");
