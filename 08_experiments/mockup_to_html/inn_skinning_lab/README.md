# Inn Screen Skinning Lab v0.1.0 — isolated visual baseline

## 核心說明

- **本資料夾定位**：一個完全獨立 (isolated)、僅使用靜態資料 (fixture-only) 的介面外觀修補實驗室 (skinning lab)。
- **非正式產物**：本資料夾內的所有程式碼皆非正式 Inn Screen runtime 的原始碼，亦不具備任何 gameplay 決策權限 (not gameplay authority)。
...
- `inn_skinning_lab.css`：複製自正式 prototype 現有樣式。
- `inn_skinning_lab.js`：靜態行為控制器，移除了與 runtime 連接的 ESM 模組與 API 呼叫。
