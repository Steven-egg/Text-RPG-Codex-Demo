...
## 最新完成：GUI Runtime Bridge Workshop Weapon Equip Live MVP（local live mode）

最近 GUI live bridge 基準 commit：`6abe303 [antig] feat(gui): add workshop weapon equip bridge & align backpack presentation`。
...
- Workshop Screen：Workshop Buy Weapon MVP 可購買既有 `SHOP_INVENTORY["weapon"]` 武器但不自動裝備；Workshop Weapon Equip MVP 只允許在 live 工坊把 inventory 內持有的 weapon slot 武器裝備到 `equipment.weapon`。

Workshop Weapon Equip Live MVP 已完成受限 live bridge 收斂：

- Workshop live「我的裝備」tab 會以玩家實際 runtime state 顯示 inventory 裝備 + 目前已裝備裝備，同品項合併並標示已裝備狀態。
- `equip_weapon` live action 只限既有 `EQUIPMENT` 且 `slot == "weapon"` 的武器；防具、飾品、特殊欄、卸下、比較、強化與完整 equipment management 仍未開放。
- gameplay 判斷在 Python server-side：檢查裝備存在、slot、inventory 持有、職業可用，以及是否已裝備同一武器；成功後重用既有 `game.equip_item(state, item_id, quiet=True)`。
- 換裝不扣 Gold；舊武器會依既有 runtime 行為回到 inventory，新武器成為 `state["equipment"]["weapon"]`，能力值由既有 `get_stats()` 自然反映。
- World Map / main inventory overlay 已對齊背包 / 裝備呈現語意：顯示 inventory 內裝備 + 目前已裝備裝備，同品項合併並標示已裝備；inventory preview 隱藏底部「返回地圖」按鈕，其他 preview 不受影響。
- 前端 JavaScript 只透過 `runtimeClient` dispatch UIAction 並 render 回傳 ScreenModel；static fixture fallback 與 UIAction logging 保留。
- 使用者手測確認：購買不自動裝備、換裝不再扣款、舊武器回 inventory、新武器進 `equipment.weapon`、狀態與背包顯示同步，且 World Map save / reload 後裝備與背包狀態可保留。
- 本 MVP 不代表完整 inventory / equipment system、armor / accessory / special slot management、unequip、equipment comparison、upgrade flow、save migration、data/schema changes、combat formula 或 stat rebalance 已開放。
...
- Town Hub live 可進入 Shop Screen；Shop live mode 會顯示可購買補給品、玩家 HP / MP / Gold resource strip、價格、目前持有數與購買回饋。
- `buy_item` live action 只限 `SHOP_INVENTORY["travel"]` 內既有 `ITEMS.kind == "consumable"` 的品項；目前對應小藥水、中藥水、解毒草與集中滴露，且仍遵守既有 unlock / availability。
- gameplay 判斷在 Python server-side：檢查 `item_id` 是否存在、是否屬於 travel shop、是否為 consumable、是否已解鎖、Gold 是否足夠；Gold 不足回 409 blocked，扣 Gold 與 `inventory` 加 1 也在 server-side。
- 前端 JavaScript 只透過 `runtimeClient` dispatch UIAction 並 render 回傳 ScreenModel；static fixture fallback 與 UIAction logging 保留。
- 使用者手測確認：法師路線完成 Start → Town → Guild → Shop → Potion purchase → World Map → Dungeon → Combat 的 owner-side regression smoke，並包含既有 item-use flow；未發現明顯 routing regression。
- 本 MVP 不新增完整 shop system、equipment purchase、sell、quantity selector、完整 inventory UI、Shop / Workshop / Magic Shop 共用抽象層或通用 facility framework；未修改 `03_engine/engine/game.py`、`04_data/`、`02_schema/`、`save.json` 或 combat formula。
...
目前仍不提升 Shop Buy Consumable MVP、Magic Shop Learn Magic Book MVP、Workshop Buy Weapon Live MVP 與 Workshop Weapon Equip Live MVP 之外的完整 Shop、Workshop、Synthesis、Storage、Magic Shop、Temple、Relic Preview 到正式 bridge contract。Guild 只完成受限 clear report 登記；Dungeon / Combat 僅完成已批准的 traversal / combat loop 小切片與 Combat Skill Button Live MVP；Shop 只完成 consumable 買 1 個；Magic Shop 只完成既有魔法書學習 1 本；Workshop 只完成既有武器購買與 weapon slot 換裝，不含防具、強化、卸下、裝備比較或完整 inventory / equipment management。這不代表完整 dungeon framework、boss framework、formal magic / skill framework、target selection、combat formula / skill rebalance、完整 inventory / equipment interaction、完整 shop system、generic facility framework、完整 guild system 或正式 quest framework 已開放。

已知後續對齊點：

- Live bridge 已收斂到 Start Screen / Town Hub / Inn / World Map、受限 Dungeon / Combat loop、Guild Report MVP、Combat Skill Button Live MVP、Shop Buy Consumable MVP、Magic Shop Learn Magic Book MVP、Workshop Buy Weapon Live MVP 與 Workshop Weapon Equip Live MVP 的最小可測 GUI shell；後續仍需以單一小切片另行批准，不自動擴張其他 live bridge surface。
...
- Shop / CLI / static prototype 仍有 presentation alignment follow-up：CLI 商店 / NPC 命名與 GUI static/live shell copy 可能不一致，例如「旅人小舖－拉比」對「星燈行商鋪／特里」；Shop live 上方可能帶入較像 Town Hub summary/status 的區塊，造成 facility screen 空白偏大與內容下移。這只列為 naming / copy / layout / ScreenModel presentation polish，不是 gameplay blocker，也不得擴張成 Shop redesign。
- Workshop live 仍有 presentation / localization follow-up：例如 stat label 可能顯示 `accuracy +3` 這類未中文化欄位；這只列為 copy / localization / ScreenModel presentation polish，不是 gameplay blocker，也不得擴張成 Workshop redesign。armor / accessory / special slot、卸下、比較、強化與完整 equipment management 另屬後續 MVP，不納入目前 live contract。
...
- 下一步 UI 工作需先 read-only 確認單一小切片。已落地的 runtime bridge 只限 blessed live slice、World Map utility preview、受限 Dungeon / Combat loop、Guild Report MVP、Combat Skill Button Live MVP、Shop Buy Consumable MVP、Magic Shop Learn Magic Book MVP、Workshop Buy Weapon Live MVP 與 Workshop Weapon Equip Live MVP；不得把 Guild Report MVP 擴張成完整 guild system、正式 quest framework、reputation 或 achievement，不得把 Combat Skill Button MVP 擴張成正式 skill system、skill framework、target selection、技能重平衡或大型 combat 重構，不得把 Shop Buy Consumable MVP 擴張成完整 shop system、equipment、sell、quantity selector、完整 inventory UI 或 Shop / Workshop / Magic Shop 共用抽象層，不得把 Magic Shop Learn Magic Book MVP 擴張成完整 magic shop system、formal magic / skill framework、target selection、combat formula / skill rebalance、generic facility framework 或 broader inventory / equipment / shop system，不得把 Workshop weapon MVP 擴張成完整 workshop / equipment / armor / upgrade / full equip-management / inventory framework，也不得擴張 Synthesis、Storage、Temple、Relic、完整 boss / inventory / dungeon framework，或為 reference/mockup 圖啟動 formal asset pipeline。
