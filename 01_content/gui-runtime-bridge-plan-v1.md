...
| Glen Boss flow UX cleanup | working tree / pending commit | Persistent Guild story hint review card (with Blood Map guidance), reward key mapping, HP/MP-only resource strip, semantic metrics (danger/squad status), dynamic 3-column action-bar layout, and boss victory exploration events. | Full quest / story / dungeon / storage / workshop frameworks. |
